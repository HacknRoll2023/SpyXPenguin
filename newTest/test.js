...
// animate penguin when index finger is lower than the middle finger
function spinPenguin(time) {
    time *= 0.001; // convert time param to seconds
    penguin.rotation.x = time;
    penguin.rotation.y = time;
    // Render the scene to canvas
    renderer.render(scene, camera);
    animationId = requestAnimationFrame(spinPenguin);
}
